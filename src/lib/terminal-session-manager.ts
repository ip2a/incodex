...
export interface TerminalCreateMessage {
  ...
  cols?: unknown;
  rows?: unknown;
  _incodexBrowserSessionId?: unknown;
  _incodexBrowserTerminalSessionId?: unknown;
}
...
function normalizeCreateOrAttachRequest(
  ...
  return {
    ...
    browserSessionId: readString(message._incodexBrowserSessionId),
    browserTerminalSessionId: readString(message._incodexBrowserTerminalSessionId),
  };
}
...
function withTerminalRoutingTarget(target: TerminalRoutingTarget, message: JsonRecord): JsonRecord {
  ...
  return {
    ...message,
    ...(target.browserSessionId ? { _incodexBrowserSessionId: target.browserSessionId } : {}),
    ...(target.browserTerminalSessionId
      ? { _incodexBrowserTerminalSessionId: target.browserTerminalSessionId }
      : {}),
  };
}
...
function buildNodePtyLoadErrorMessage(error: unknown, rebuildAttempted: boolean): string {
  ...
  const instructions =
    "Run `pnpm rebuild node-pty` in the Incodex install directory (or reinstall) and restart.";
