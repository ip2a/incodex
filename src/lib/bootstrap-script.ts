...
export function renderBootstrapScript(config: BootstrapScriptConfig): string {
  return serializeInlineScript(bootstrapIncodexInBrowser, config);
}

function bootstrapIncodexInBrowser(config: BootstrapScriptConfig): void {
  ...
  type BrowserDebugEvent = {
    at: string;
    direction: "in" | "out" | "queue" | "socket";
    event: Record<string, unknown>;
  };

  type ServiceWorkerDebugState = {
    status: "pending" | "skipped-dev" | "unavailable" | "registering" | "registered" | "failed";
  ...
  const RESET_QUERY_KEY = "incodexReset";
  ...
  const LAST_ROUTE_STORAGE_KEY = "__incodex_last_route";
  ...
  const mobileSidePanelOpenButton = document.createElement("button");
  ...
  let mobileSidePanelObserver: MutationObserver | null = null;
  ...
  let serviceWorkerDebugState: ServiceWorkerDebugState = {
    status: "pending",
    scriptUrl: INCODEX_SERVICE_WORKER_PATH,
  };
  ...
  mobileSidePanelCloseButton.id = "incodex-mobile-side-panel-close";
  mobileSidePanelCloseButton.type = "button";
  mobileSidePanelCloseButton.hidden = true;
  mobileSidePanelCloseButton.setAttribute("aria-label", "Close side panel");
  mobileSidePanelCloseButton.textContent = "Close";
  mobileSidePanelOpenButton.id = "incodex-mobile-side-panel-open";
  ...
  runWhenDocumentReady(() => {
    ...
    installMobileSidePanelCoordination();
  ...
  function installCryptoRandomUuidFallback(): void {
    ...
    try {
      ...
      try {
        (cryptoObject as Crypto & { randomUUID?: Crypto["randomUUID"] }).randomUUID = randomUUID;
      } catch {
        // Continue without a fallback if the browser exposes an immutable crypto object.
      }
    }
  }

  function ensureHostAttached(host: HTMLElement): void {
    if (!document.body) {
      runWhenDocumentReady(() => {
        ensureStylesheetLink(config.stylesheetHref);
  ...
  function ensureStylesheetLink(href?: string): HTMLLinkElement | null {
    ...
    const current = document.getElementById(INCODEX_STYLESHEET_ID);
  ...
  function installDebugConsoleApi(): void {
    ...
    targetWindow.__incodexDebug = {
      enable() {
        getStorage("localStorage")?.setItem(DEBUG_STORAGE_KEY, "1");
        browserDebugLog("debug enabled", { source: "console" });
      },
      disable() {
        getStorage("localStorage")?.setItem(DEBUG_STORAGE_KEY, "0");
        console.info("[incodex:browser] debug disabled");
      },
      resetStartupState() {
        resetClientStartupState();
        console.info("[incodex:browser] startup state cleared; reloading");
        window.location.reload();
      },
      status() {
        const status = {
          enabled: isBrowserDebugEnabled(),
          devMode: config.devMode === true,
        };
        console.info("[incodex:browser] debug status", status);
        return status;
      },
      snapshot() {
        const snapshot = getBrowserDebugSnapshot();
        console.info("[incodex:browser] debug snapshot", snapshot);
        return snapshot;
      },
    };

    browserDebugLog("debug console api installed", {
      enabled: isBrowserDebugEnabled(),
      devMode: config.devMode === true,
    });
  }

  function getBrowserDebugSnapshot(): Record<string, unknown> {
    return {
      enabled: isBrowserDebugEnabled(),
      devMode: config.devMode === true,
      href: window.location.href,
      visibilityState: document.visibilityState,
      hasDocumentFocus: hasDocumentFocus(),
      ...
      socketReadyState: socket ? describeSocketReadyState(socket.readyState) : "none",
      lastServerHeartbeatAgeMs:
        lastServerHeartbeatAt > 0 ? Date.now() - lastServerHeartbeatAt : null,
      serviceWorker: serviceWorkerDebugState,
      recentEvents: recentDebugEvents.slice(),
    };
  }

  function describeSocketReadyState(readyState: number): string {
    switch (readyState) {
  ...
  function installGlobalErrorDebugHandlers(): void {
    window.addEventListener("error", (event) => {
      browserDebugLog("window error", {
        message: event.message,
        filename: event.filename,
        lineno: event.lineno,
        colno: event.colno,
        error: summarizeBrowserError(event.error),
      });
    });

    window.addEventListener("unhandledrejection", (event) => {
      browserDebugLog("unhandled promise rejection", {
        reason: summarizeBrowserError(event.reason),
      });
    });
  }

  function summarizeBrowserError(error: unknown): Record<string, unknown> {
    ...
    if (isRecord(error)) {
      ...
      for (const key of ["name", "message", "stack", "code", "type"]) {
      ...
      return Object.keys(summary).length > 0
        ? summary
        : { value: sanitizeBrowserDebugValue(error, 0) };
    }
  ...
  function isBrowserDebugEnabled(): boolean {
    const override = readBrowserDebugOverride();
    if (override !== null) {
      return override;
    }
  ...
  function readBrowserDebugOverride(): boolean | null {
    try {
      const queryValue = new URL(window.location.href).searchParams.get(DEBUG_QUERY_KEY);
      const queryOverride = parseDebugFlag(queryValue);
      if (queryOverride !== null) {
        return queryOverride;
      }
    } catch {
      // Continue with storage/default debug configuration.
    }

    const storageOverride = parseDebugFlag(getStorage("localStorage")?.getItem(DEBUG_STORAGE_KEY));
  ...
  function parseDebugFlag(value: string | null | undefined): boolean | null {
  ...
  function browserDebugLog(message: string, details?: unknown): void {
    if (!isBrowserDebugEnabled()) {
      return;
    }

    if (details === undefined) {
      console.debug(`[incodex:browser] ${message}`);
      return;
    }

    console.debug(`[incodex:browser] ${message}`, sanitizeBrowserDebugValue(details, 0));
  }

  function rememberBrowserDebugEvent(
    direction: BrowserDebugEvent["direction"],
    event: Record<string, unknown>,
  ): void {
    ...
    if (recentDebugEvents.length > 80) {
      recentDebugEvents.splice(0, recentDebugEvents.length - 80);
    }
  }

  function sanitizeBrowserDebugValue(value: unknown, depth: number): unknown {
    ...
    if (Array.isArray(value)) {
      return value.slice(0, 20).map((item) => sanitizeBrowserDebugValue(item, depth + 1));
    }
    ...
    for (const [key, entryValue] of entries.slice(0, 20)) {
      ...
      sanitized[key] = sanitizeBrowserDebugValue(entryValue, depth + 1);
    }
    if (entries.length > 20) {
      sanitized.__truncatedKeys = entries.length - 20;
    }
    return sanitized;
  }

  function summarizeBrowserMessage(message: unknown): Record<string, unknown> {
    if (!isRecord(message)) {
      return {
    ...
    if ("body" in message) {
      summary.hasBody = message.body !== undefined;
    }
    return summary;
  }

  function summarizeBrowserEnvelope(envelope: BrowserToServerEnvelope): Record<string, unknown> {
    switch (envelope.type) {
      case "bridge_message":
        return {
          type: envelope.type,
          message: summarizeBrowserMessage(envelope.message),
        };
      case "worker_message":
        return {
          type: envelope.type,
  ...
  function summarizeServerEnvelope(envelope: ServerToBrowserEnvelope): Record<string, unknown> {
    switch (envelope.type) {
      case "bridge_message":
        return {
          type: envelope.type,
      ...
      case "session_revoked":
        return {
          type: envelope.type,
          message: "message" in envelope ? envelope.message : envelope.reason,
        };
      case "css_reload":
        return {
          type: envelope.type,
  ...
  function resetClientStartupStateIfRequested(): void {
    ...
    if (!isTruthyQueryFlag(currentUrl.searchParams.get(RESET_QUERY_KEY))) {
      return;
    }
  ...
  function resetClientStartupState(): void {
  ...
  async function handleBrowserBackedFetch(message: unknown): Promise<boolean> {
    ...
    const codexFetchPath = readCodexFetchPath(message.url);
    if (codexFetchPath !== "pick-files") {
      return false;
    }

    try {
      const params = readCodexFetchParams(message.body);
      const imagesOnly = isRecord(params) && params.imagesOnly === true;
      const files = await openBrowserFilePicker({ imagesOnly });
      const uploadedFiles = [];
      for (const file of files) {
        uploadedFiles.push(await uploadBrowserPickedFile(file));
      }

      browserDebugLog("browser pick-files success", {
        requestId: message.requestId,
        count: uploadedFiles.length,
        imagesOnly,
      });
      dispatchCodexFetchSuccess(message.requestId, {
        files: uploadedFiles,
      });
    } catch (error) {
      const messageText = error instanceof Error ? error.message : String(error);
      browserDebugLog("browser pick-files failed", {
        requestId: message.requestId,
        error: messageText,
      });
      dispatchCodexFetchError(message.requestId, 500, messageText || "Unable to pick files.");
    }

    return true;
  }

  function readCodexFetchPath(url: string): string | null {
  ...
  function readCodexFetchParams(body: unknown): unknown {
    ...
    return isRecord(parsedBody) && isRecord(parsedBody.params) ? parsedBody.params : parsedBody;
  }

  function parseMaybeJsonBody(body: unknown): unknown {
  ...
  function openBrowserFilePicker({ imagesOnly }: { imagesOnly: boolean }): Promise<File[]> {
  ...
  async function uploadBrowserPickedFile(file: File): Promise<{
    label: string;
    path: string;
    fsPath: string;
  }> {
    ...
    if (!response.ok || !isRecord(result) || typeof result.path !== "string") {
      const error =
    ...
    return {
      label: file.name || result.path,
      path: result.path,
      fsPath: result.path,
    };
  }

  function dispatchCodexFetchSuccess(requestId: string, body: unknown, status = 200): void {
    dispatchHostMessage({
      type: "fetch-response",
      requestId,
  ...
  function dispatchCodexFetchError(requestId: string, status: number, error: string): void {
    dispatchHostMessage({
      type: "fetch-response",
      requestId,
      responseType: "error",
  ...
  function reloadStylesheet(href: string): void {
    ...
    nextLink.id = INCODEX_STYLESHEET_ID;
    ...
    nextLink.addEventListener(
      ...
      () => {
        nextLink.remove();
        showNotice("Failed to reload Incodex CSS.");
      },
      { once: true },
    );
  ...
  function showNotice(message: string): void {
    ...
    toast.dataset.incodexToast = "true";
    ensureHostAttached(toastHost);
  ...
  function setConnectionStatus(message: string, options: ConnectionStatusOptions = {}): void {
    ...
    card.dataset.incodexStatusCard = "true";

    const title = document.createElement("strong");
    title.textContent = "Incodex";
    ...
    if (options.actions && options.actions.length > 0) {
      ...
      actions.dataset.incodexStatusActions = "true";

      for (const action of options.actions) {
        const button = document.createElement("button");
        button.type = "button";
        button.textContent = action.label;
        button.dataset.incodexStatusAction = action.id;
        button.dataset.incodexStatusStyle = action.style ?? "secondary";
  ...
  function installMobileSidebarThreadNavigationClose(): void {
    syncMobileSidebarOverlayState(false);
    ...
    document.addEventListener("pointerover", handleMobileSidebarHoverIntent, true);
    document.addEventListener("pointerenter", handleMobileSidebarHoverIntent, true);
    document.addEventListener("mouseover", handleMobileSidebarHoverIntent, true);
    window.addEventListener("resize", handleMobileSidebarOverlayLayoutChange);
  }

  function installMobileSidePanelCoordination(): void {
    syncMobileSidePanelState();
    mobileSidePanelCloseButton.addEventListener("click", closeMobileSidePanel);
    mobileSidePanelOpenButton.addEventListener("click", reopenMobileSidePanel);
    document.addEventListener("click", handleMobileSidePanelCloseClick, true);
    document.addEventListener("click", handleMobileSidePanelOpenClick, true);
    document.addEventListener("click", handleMobileSidePanelToggleClick, true);
    document.addEventListener("click", handleMobilePanelBackdropClick, true);
    window.addEventListener("resize", handleMobileSidePanelLayoutChange);

    if (mobileSidePanelObserver || typeof MutationObserver !== "function") {
      return;
    }

    const target = document.body ?? document.documentElement;
    mobileSidePanelObserver = new MutationObserver(() => {
      scheduleMobileSidePanelSync();
    });
    mobileSidePanelObserver.observe(target, {
      childList: true,
      subtree: true,
      attributes: true,
      attributeFilter: ["aria-label", "class", "data-state", "hidden", "style", "title"],
    });
  }

  function installSidebarModePersistence(): void {
    sidebarModeFromHost = readSidebarModeFromBrowserStorage();
    hasReceivedSidebarModeSync = sidebarModeFromHost !== null || isMobileSidebarViewport();
  ...
  function installRouteDatasetSync(): void {
    ...
    const historyObject = window.history as History & {
      __incodexRouteSyncInstalled?: boolean;
    };
    if (historyObject.__incodexRouteSyncInstalled) {
      return;
    }

    historyObject.__incodexRouteSyncInstalled = true;
  ...
  function syncRouteDataset(): void {
    document.documentElement.dataset.incodexRoute = readCurrentPathname();
  }
  ...
  function syncSettingsShellPresence(): void {
    ...
    if (hasSettingsShell) {
      document.documentElement.dataset.incodexSettingsShell = "true";
    } else {
      delete document.documentElement.dataset.incodexSettingsShell;
    }
  }
  ...
  function handleMobileSidebarOverlayLayoutChange(): void {
    if (isMobileSidebarViewport()) {
      syncMobileSidebarOverlayState(isMobileSidebarOverlayOpen());
      return;
    }

    delete document.documentElement.dataset.incodexMobileSidebar;
    clearPendingSidebarModeTarget();
  }
  ...
  function handleMobileSidebarToggleClick(event: MouseEvent): void {
    if (!isMobileSidebarViewport() || !isPrimaryUnmodifiedClick(event)) {
      return;
    }
    ...
    const nextOpenState = !isMobileSidebarOverlayOpen();
    setMobileSidebarOverlayOpen(nextOpenState);
  }

  function setMobileSidebarOverlayOpen(isOpen: boolean): void {
    if (!isMobileSidebarViewport()) {
      syncMobileSidebarOverlayState(false);
      return;
    }
    ...
    if (isOpen) {
      ...
      syncMobileSidebarOverlayState(true);
      ensureMobileSidebarHostMode("expanded");
      scheduleMobileSidebarHostModeVerification("expanded");
      return;
    }

    syncMobileSidebarOverlayState(false);
    ensureMobileSidebarHostMode("collapsed");
    scheduleMobileSidebarHostModeVerification("collapsed");
  }

  function ensureMobileSidebarHostMode(mode: SidebarMode): void {
    if (isMobileSidebarViewport()) {
      requestMobileSidebarHostMode(mode);
      return;
    }
    ...
    armSidebarModeInteraction();
    notePendingSidebarModeTarget(mode);
    dispatchHostMessage({ type: "toggle-sidebar" });
  }

  function requestMobileSidebarHostMode(mode: SidebarMode): void {
    const currentMode = readMobileSidebarHostMode();
    if (currentMode === mode) {
      if (pendingSidebarModeTarget === mode) {
        clearPendingSidebarModeTarget();
      }
      return;
    }

    if (currentMode === null && mode === "collapsed") {
      return;
    }

    if (pendingSidebarModeTarget === mode && Date.now() < pendingSidebarModeTargetUntil) {
      return;
    }
  ...
  function scheduleMobileSidebarHostModeVerification(mode: SidebarMode): void {
    [180, 650, 1_300].forEach((delayMs) => {
      window.setTimeout(() => {
        if (!isMobileSidebarViewport() || getDesiredSidebarMode() !== mode) {
          return;
        }

        const currentMode = updateMobileSidebarHostDataset();
        if (currentMode === mode) {
          clearPendingSidebarModeTarget();
          return;
        }
  ...
  function handleMobileSidebarHoverIntent(event: Event): void {
    if (!isMobileSidebarViewport()) {
      return;
    }

    const target = event.target instanceof Element ? event.target : null;
    if (!target || !isInMobileSidebarHoverHotZone(target)) {
      return;
    }
  ...
  function isInMobileSidebarHoverHotZone(element: Element): boolean {
    if (element.closest('.app-shell-left-panel, nav[role="navigation"]')) {
      return true;
    }

    const nearestInteractive = element.closest('button, a, [role="button"]');
    return nearestInteractive instanceof Element && isSidebarToggleTrigger(nearestInteractive);
  }

  function handleMobileSidePanelCloseClick(event: MouseEvent): void {
    if (!isMobileSidebarViewport() || !isPrimaryUnmodifiedClick(event)) {
      return;
    }

    const target = event.target instanceof Element ? event.target : null;
    if (!target || !target.closest("#incodex-mobile-side-panel-close")) {
      return;
    }
  ...
  function handleMobileSidePanelOpenClick(event: MouseEvent): void {
    if (!isMobileSidebarViewport() || !isPrimaryUnmodifiedClick(event)) {
      return;
    }

    const target = event.target instanceof Element ? event.target : null;
    if (!target || !target.closest("#incodex-mobile-side-panel-open")) {
      return;
    }
  ...
  function handleMobileSidePanelToggleClick(event: MouseEvent): void {
    if (!isMobileSidebarViewport() || !isPrimaryUnmodifiedClick(event)) {
      return;
    }
    ...
    if (isMobileSidePanelDismissed && panelRoot) {
      event.preventDefault();
      stopEventPropagation(event);
      openDismissedMobileSidePanel(panelRoot);
      return;
    }

    isMobileSidePanelDismissed = false;
    if (isMobileSidebarOverlayOpen() || isMobileSidebarOpen()) {
      scheduleMobileSidebarClose();
    }
    closeTransientMenus();
    scheduleMobileSidePanelSync(40);
    window.setTimeout(() => {
      scheduleMobileSidePanelSync(20);
    }, 180);
  }

  function handleMobilePanelBackdropClick(event: MouseEvent): void {
    if (!isMobileSidebarViewport() || !isPrimaryUnmodifiedClick(event)) {
      return;
    }

    const target = event.target instanceof Element ? event.target : null;
    if (!target || !target.closest(".main-surface")) {
      return;
    }
    if (target.closest('[data-incodex-mobile-right-panel-root="true"]')) {
      return;
    }
    if (!isMobileSidebarOverlayOpen() && !isMobileSidePanelOpen()) {
      return;
    }
  ...
  function handleMobileSidePanelLayoutChange(): void {
    if (!isMobileSidebarViewport()) {
      clearMobileSidePanelState();
      return;
    }
  ...
  function scheduleMobileSidePanelSync(delay = 20): void {
    if (mobileSidePanelSyncTimer !== null) {
      window.clearTimeout(mobileSidePanelSyncTimer);
    }

    mobileSidePanelSyncTimer = window.setTimeout(() => {
      mobileSidePanelSyncTimer = null;
      syncMobileSidePanelState();
    }, delay);
  }

  function syncMobileSidePanelState(): void {
    if (!isMobileSidebarViewport()) {
      clearMobileSidePanelState();
      mobileSidePanelCloseButton.hidden = true;
      mobileSidePanelOpenButton.hidden = true;
    ...
    if (isMobileSidePanelDismissed) {
      ...
      if (dismissedPanelRoot) {
        tagMobileSidePanelRoot(dismissedPanelRoot);
        document.documentElement.dataset.incodexMobileRightPanel = "closed";
        mobileSidePanelCloseButton.hidden = true;
        mobileSidePanelOpenButton.hidden = false;
        return;
      }
      isMobileSidePanelDismissed = false;
    }

    const panelRoot = findMobileSidePanelRoot();
    if (!panelRoot) {
      isMobileSidePanelDismissed = false;
      document.documentElement.dataset.incodexMobileRightPanel = "closed";
      clearMobileSidePanelRootTags();
      mobileSidePanelCloseButton.hidden = true;
      mobileSidePanelOpenButton.hidden = true;
      return;
    }
    ...
    mobileSidePanelCloseButton.hidden = false;
    mobileSidePanelOpenButton.hidden = true;
  ...
  function reopenMobileSidePanel(): void {
    const panelRoot = findExistingMobileSidePanelRoot() ?? findMobileSidePanelRoot();
    if (!panelRoot) {
      isMobileSidePanelDismissed = false;
      document.documentElement.dataset.incodexMobileRightPanel = "closed";
  ...
  function tagMobileSidePanelRoot(panelRoot: HTMLElement): void {
    const taggedRoots = document.querySelectorAll('[data-incodex-mobile-right-panel-root="true"]');
    for (let index = 0; index < taggedRoots.length; index += 1) {
      const taggedRoot = taggedRoots.item(index);
      if (taggedRoot !== panelRoot) {
        taggedRoot.removeAttribute("data-incodex-mobile-right-panel-root");
      }
    }

    if (panelRoot.getAttribute("data-incodex-mobile-right-panel-root") !== "true") {
      panelRoot.setAttribute("data-incodex-mobile-right-panel-root", "true");
    }
  }

  function clearMobileSidePanelState(): void {
    delete document.documentElement.dataset.incodexMobileRightPanel;
    clearMobileSidePanelRootTags();
    mobileSidePanelCloseButton.hidden = true;
    mobileSidePanelOpenButton.hidden = true;
  }

  function clearMobileSidePanelRootTags(): void {
    const taggedRoots = document.querySelectorAll('[data-incodex-mobile-right-panel-root="true"]');
    for (let index = 0; index < taggedRoots.length; index += 1) {
      taggedRoots.item(index).removeAttribute("data-incodex-mobile-right-panel-root");
    }
  }

  function isMobileSidePanelOpen(): boolean {
    return document.documentElement.dataset.incodexMobileRightPanel === "open";
  }

  function closeMobileSidePanel(): void {
    if (isClosingMobileSidePanel) {
      return;
    }
    ...
    try {
      ...
      window.setTimeout(() => {
        isClosingMobileSidePanel = false;
      }, 0);
    }
  }

  function isMobileSidePanelToggleTrigger(element: Element): boolean {
    const label = normalizeElementLabel(element);
    if (!label) {
      return false;
    }
    ...
    return (
      ...
      label.includes("toggle side panel")
    );
  }

  function findExistingMobileSidePanelRoot(): HTMLElement | null {
    const taggedRoot = document.querySelector('[data-incodex-mobile-right-panel-root="true"]');
    if (taggedRoot instanceof HTMLElement) {
      return taggedRoot;
    }

    const sidePanel = document.querySelector(".main-surface aside");
    return sidePanel instanceof HTMLElement ? sidePanel : null;
  }

  function findMobileSidePanelRoot(): HTMLElement | null {
    ...
    for (let index = 0; index < elements.length; index += 1) {
      ...
      if (!(element instanceof HTMLElement) || !isMobileSidePanelRootCandidate(element)) {
        continue;
      }

      const rect = element.getBoundingClientRect();
      const rightEdgeDistance = Math.abs(viewportWidth - rect.right);
      const widthScore = Math.min(rect.width / Math.max(viewportWidth, 1), 1);
      const heightScore = Math.min(rect.height / Math.max(viewportHeight, 1), 1);
      const edgeScore = Math.max(0, 1 - rightEdgeDistance / Math.max(viewportWidth, 1));
      const semanticScore = getMobileSidePanelSemanticScore(element);
      const score = edgeScore * 3 + heightScore * 2 + widthScore + semanticScore;
  ...
  function isMobileSidePanelRootCandidate(element: HTMLElement): boolean {
    if (element === document.body || element === document.documentElement) {
      return false;
    }
    if (element.matches('.main-surface, .app-shell-left-panel, nav[role="navigation"]')) {
      return false;
    }
    if (element.querySelector('.main-surface, .app-shell-left-panel, nav[role="navigation"]')) {
      return false;
    }
    if (
      element.closest(
        '.app-shell-left-panel, nav[role="navigation"], #incodex-toast-host, #incodex-status-host',
      )
    ) {
      return false;
    }
    ...
    const viewportHeight = window.innerHeight || document.documentElement.clientHeight || 0;
    const rect = element.getBoundingClientRect();
    if (rect.width < 160 || rect.height < Math.min(360, viewportHeight * 0.45)) {
      return false;
    }
    if (element.tagName !== "ASIDE" && rect.width > viewportWidth * 0.96) {
      return false;
    }
    if (rect.left >= viewportWidth - 24 || rect.right <= viewportWidth * 0.45) {
      return false;
    }
    if (Math.abs(viewportWidth - rect.right) > Math.max(48, viewportWidth * 0.18)) {
      return false;
    }
    if (rect.top > viewportHeight * 0.35 || rect.bottom < viewportHeight * 0.55) {
      return false;
    }

    return getMobileSidePanelSemanticScore(element) > 0;
  }

  function getMobileSidePanelSemanticScore(element: Element): number {
    const label = normalizeElementLabel(element);
    const text = (element.textContent ?? "").trim().toLowerCase();
    const combined = `${label} ${text}`;
    let score = 0;
    if (/\b(files?|browser|terminal|side chat|diff|logs?)\b/.test(combined)) {
      score += 2;
    }
    if (
      element.querySelector(
        'button, [role="tab"], [role="tabpanel"], input, textarea, [contenteditable="true"]',
      )
    ) {
      score += 1;
    }
    if (
      element.querySelector(
        '[aria-label*="Terminal"], [aria-label*="Filter files"], [placeholder*="Filter files"], [placeholder="Enter a URL"]',
      )
    ) {
      score += 2;
    }
  ...
  function normalizeElementLabel(element: Element): string {
  ...
  function scheduleMobileSidebarClose(): void {
    window.setTimeout(() => {
      setMobileSidebarOverlayOpen(false);
    }, 0);
  }

  function syncMobileSidebarOverlayState(isOpen: boolean): void {
    if (!isMobileSidebarViewport()) {
      delete document.documentElement.dataset.incodexMobileSidebar;
      delete document.documentElement.dataset.incodexMobileSidebarHost;
      return;
    }

    document.documentElement.dataset.incodexMobileSidebar = isOpen ? "open" : "closed";
    updateMobileSidebarHostDataset();
  }

  function updateMobileSidebarHostDataset(): SidebarMode | null {
    if (!isMobileSidebarViewport()) {
      delete document.documentElement.dataset.incodexMobileSidebarHost;
      return null;
    }

    const mode = readMobileSidebarHostMode();
    document.documentElement.dataset.incodexMobileSidebarHost = mode ?? "unknown";
    return mode;
  }

  function isMobileSidebarOverlayOpen(): boolean {
    return document.documentElement.dataset.incodexMobileSidebar === "open";
  }
  ...
  function getDesiredSidebarMode(): SidebarMode {
    if (isMobileSidebarViewport()) {
      return isMobileSidebarOverlayOpen() ? "expanded" : "collapsed";
    }
  ...
  function reconcileSidebarMode(retriesRemaining: number): void {
    if (isMobileSidebarViewport()) {
      const currentMode = updateMobileSidebarHostDataset();
      if (pendingSidebarModeTarget && currentMode === pendingSidebarModeTarget) {
        clearPendingSidebarModeTarget();
      }
      clearSidebarModeInteractionArm();
      hasReceivedSidebarModeSync = true;
    ...
    const desiredMode = getDesiredSidebarMode();
    const currentMode = readSidebarMode();
    if (!currentMode) {
      if (retriesRemaining > 0) {
        scheduleSidebarModeReconcile(retriesRemaining - 1, 50);
  ...
  function readSidebarMode(): SidebarMode | null {
    if (isMobileSidebarViewport()) {
      return readMobileSidebarHostMode();
    }
  ...
  function readMobileSidebarHostMode(): SidebarMode | null {
    const contentPane = document.querySelector(".main-surface");
    if (!(contentPane instanceof Element)) {
      return readSidebarToggleLabelMode();
    }
    ...
    if (width !== "" || transform !== "") {
      ...
      const transformIndicatesOpen =
        transform !== "" && transform !== "translateX(0)" && transform !== "translateX(0px)";
      return widthIndicatesOpen || transformIndicatesOpen ? "expanded" : "collapsed";
    }

    const toggleMode = readSidebarToggleLabelMode();
    if (toggleMode) {
      return toggleMode;
    }
    ...
    if (typeof contentPane.getBoundingClientRect === "function") {
      ...
      if (
        rect.left > 0.5 ||
        (viewportWidth > 0 && rect.width > 0 && rect.width < viewportWidth - 0.5)
      ) {
        return "expanded";
      }
    }
    ...
    const navigation = document.querySelector('nav[role="navigation"]');
    if (navigation instanceof Element && typeof navigation.getBoundingClientRect === "function") {
      const rect = navigation.getBoundingClientRect();
      if (rect.width > 0.5 && rect.right > 0.5) {
        return "expanded";
      }
    }
  ...
  function readSidebarToggleLabelMode(): SidebarMode | null {
    ...
    const ariaLabel = toggle.getAttribute("aria-label")?.trim().toLowerCase() ?? "";
  ...
  function tagOpenInAppButtons(root: Document | Element = document): void {
    root.querySelectorAll("div.inline-flex").forEach((group) => {
      ...
      if (isOpenInAppButtonGroup(group)) {
        group.dataset.incodexOpenInApp = "true";
        return;
      }
      delete group.dataset.incodexOpenInApp;
    });
  }
  ...
  function renderWorkspaceRootPicker(): void {
    ...
    backdrop.dataset.incodexWorkspaceRootPickerBackdrop = "true";

    const dialog = document.createElement("section");
    dialog.dataset.incodexWorkspaceRootPickerDialog = "true";

    const header = document.createElement("div");
    header.dataset.incodexWorkspaceRootPickerHeader = "true";
    ...
    subtitle.textContent =
      state.context === "onboarding"
        ? "Choose or create a folder on the Incodex host to start working locally."
        : "Choose or create a folder on the Incodex host to add it as a project.";

    header.append(title, subtitle);

    const pathForm = document.createElement("div");
    pathForm.dataset.incodexWorkspaceRootPickerPathForm = "true";

    const pathLabel = document.createElement("label");
    pathLabel.dataset.incodexWorkspaceRootPickerPathLabel = "true";
    ...
    if (state.errorMessage) {
      ...
      errorText.dataset.incodexWorkspaceRootPickerError = "true";
      errorText.textContent = state.errorMessage;
      content.appendChild(errorText);
    }

    const list = document.createElement("div");
    list.dataset.incodexWorkspaceRootPickerList = "true";
    if (state.isLoading) {
      const loading = document.createElement("p");
      loading.dataset.incodexWorkspaceRootPickerEmpty = "true";
      ...
      if (rows.length === 0) {
        ...
        empty.dataset.incodexWorkspaceRootPickerEmpty = "true";
      ...
      for (const rowConfig of rows) {
        ...
        row.type = "button";
        row.dataset.incodexWorkspaceRootPickerRow = "true";
        if (rowConfig.isParent) {
          row.dataset.incodexWorkspaceRootPickerParentRow = "true";
        }
    ...
    footer.dataset.incodexWorkspaceRootPickerFooter = "true";

    const cancelButton = document.createElement("button");
    cancelButton.type = "button";
    cancelButton.dataset.incodexWorkspaceRootPickerCancelButton = "true";
    ...
    useFolderButton.dataset.incodexWorkspaceRootPickerUseFolderButton = "true";
  ...
  async function loadWorkspaceRootPickerPath(path: string): Promise<void> {
    ...
    try {
      const result = await callIncodexIpc("workspace-root-picker/list", {
        path,
      });
  ...
  async function createWorkspaceRootPickerDirectory(): Promise<void> {
    ...
    try {
      ...
      const result = await callIncodexIpc("workspace-root-picker/create-directory", {
        parentPath,
        name,
      });
  ...
  async function confirmWorkspaceRootPickerSelection(): Promise<void> {
    ...
    try {
      const result = await callIncodexIpc("workspace-root-picker/confirm", {
  ...
  async function cancelWorkspaceRootPicker(): Promise<void> {
    ...
    try {
      await callIncodexIpc("workspace-root-picker/cancel", {
        context: state.context,
      });
      closeWorkspaceRootPicker();
    } catch (error) {
  ...
  function openWorkspaceRootDialog(mode: WorkspaceRootDialogMode): void {
    ...
    const backdrop = document.createElement("div");
    backdrop.dataset.incodexImportBackdrop = "true";

    const dialog = document.createElement("section");
    dialog.dataset.incodexImportDialog = "true";

    const header = document.createElement("div");
    header.dataset.incodexImportHeader = "true";
    ...
    subtitle.textContent =
      mode === "pick"
        ? "Browse the host filesystem and open the current folder in Incodex."
        : "Browse the host filesystem and add the current folder as another project in Incodex.";

    header.append(title, subtitle);

    const browserShell = document.createElement("div");
    browserShell.dataset.incodexWorkspaceBrowser = "true";

    const sidebar = document.createElement("aside");
    sidebar.dataset.incodexWorkspaceSidebar = "true";

    const sidebarTitle = document.createElement("p");
    sidebarTitle.dataset.incodexWorkspaceSidebarTitle = "true";
    sidebarTitle.textContent = "Folder tree";

    const tree = document.createElement("ul");
    tree.dataset.incodexWorkspaceTree = "true";

    sidebar.append(sidebarTitle, tree);

    const mainPanel = document.createElement("section");
    mainPanel.dataset.incodexWorkspaceMain = "true";

    const mobileLocation = document.createElement("div");
    mobileLocation.dataset.incodexWorkspaceMobileLocation = "true";

    const mobileLocationSummary = document.createElement("div");
    mobileLocationSummary.dataset.incodexWorkspaceMobileLocationSummary = "true";

    const mobileLocationName = document.createElement("strong");
    mobileLocationName.dataset.incodexWorkspaceMobileLocationName = "true";

    const mobileLocationToggle = document.createElement("button");
    mobileLocationToggle.type = "button";
    mobileLocationToggle.dataset.incodexWorkspaceMobileToggle = "true";

    mobileLocationSummary.append(mobileLocationName, mobileLocationToggle);

    const mobileLocationPath = document.createElement("div");
    mobileLocationPath.dataset.incodexWorkspaceMobilePath = "true";

    const mobileLocationPathValue = document.createElement("code");

    const mobileLocationPathNav = document.createElement("div");
    mobileLocationPathNav.dataset.incodexWorkspaceMobilePathNav = "true";

    const mobileUpButton = document.createElement("button");
    mobileUpButton.type = "button";
    mobileUpButton.dataset.incodexWorkspaceNavButton = "true";
    mobileUpButton.textContent = "Up";

    const mobileBreadcrumb = document.createElement("nav");
    mobileBreadcrumb.dataset.incodexWorkspaceBreadcrumb = "true";
    ...
    toolbar.dataset.incodexWorkspaceToolbar = "true";

    const upButton = document.createElement("button");
    upButton.type = "button";
    upButton.dataset.incodexWorkspaceNavButton = "true";
    upButton.textContent = "Up";

    const breadcrumb = document.createElement("nav");
    breadcrumb.dataset.incodexWorkspaceBreadcrumb = "true";

    toolbar.append(upButton, breadcrumb);

    const currentFolder = document.createElement("div");
    currentFolder.dataset.incodexWorkspaceCurrent = "true";
    ...
    listHeader.dataset.incodexWorkspaceListHeader = "true";
    ...
    listBody.dataset.incodexWorkspaceList = "true";

    const status = document.createElement("p");
    status.dataset.incodexWorkspaceStatus = "true";

    mainPanel.append(toolbar, currentFolder, listHeader, listBody, status);
    browserShell.append(sidebar, mainPanel);

    const actions = document.createElement("div");
    actions.dataset.incodexImportActions = "true";
    ...
    const render = () => {
      ...
      for (const entry of currentDirectory.entries) {
        const row = document.createElement("button");
        row.type = "button";
        row.dataset.incodexWorkspaceEntry = "true";
  ...
  async function callIncodexIpc(method: string, params?: unknown): Promise<unknown> {
    const response = await nativeFetch("/ipc-request", {
      ...
      credentials: "same-origin",
      body: JSON.stringify({
        requestId: `incodex-ipc-${++nextIpcRequestId}`,
        method,
        params,
      }),
    });
  ...
  async function addWorkspaceRootFromHost(params: {
    ...
    const result = await callIncodexIpc("workspace-root-option/add", params);
    if (!isWorkspaceRootAddResult(result)) {
      throw new Error("Incodex returned an invalid workspace-root response.");
    }
    return result;
  }
  ...
  async function listWorkspaceRootBrowserFromHost(
    ...
    const result = await callIncodexIpc("workspace-root-browser/list", root ? { root } : undefined);
    if (!isWorkspaceRootBrowserResult(result)) {
      throw new Error("Incodex returned an invalid workspace-root browser response.");
    }
    return result;
  }
  ...
  function createWorkspaceRootTreeItem(
    ...
    item.dataset.incodexWorkspaceTreeItem = "true";

    const button = document.createElement("button");
    button.type = "button";
    button.dataset.incodexWorkspaceTreeButton = "true";
    ...
    caret.dataset.incodexWorkspaceTreeCaret = "true";
    ...
    if (isExpanded && currentDirectory && currentDirectory.entries.length > 0) {
      ...
      subtree.dataset.incodexWorkspaceTree = "true";
      for (const entry of currentDirectory.entries) {
        if (
  ...
  function handleIncodexBridgeMessage(message: unknown): boolean {
    if (!isRecord(message) || typeof message.type !== "string") {
      return false;
    }

    if (message.type === "incodex-open-workspace-root-picker") {
    ...
    if (message.type === "incodex-open-workspace-root-dialog") {
  ...
  function normalizeBrowserUrlForRefresh(): void {
    const currentUrl = new URL(window.location.href);
    if (readThreadQueryConversationId(currentUrl) !== null) {
      clearThreadQuery();
      return;
    }
  ...
  async function registerPwaServiceWorker(): Promise<void> {
    ...
    if (
      ...
      browserDebugLog("service worker unavailable");
      rememberBrowserDebugEvent("socket", {
        type: "service-worker-unavailable",
      });
      return;
    }
    ...
    try {
      ...
      browserDebugLog("service worker register start", {
        scriptUrl: INCODEX_SERVICE_WORKER_PATH,
      });
      const registration = await navigatorObject.serviceWorker.register(
        INCODEX_SERVICE_WORKER_PATH,
        {
          scope: "/",
          updateViaCache: "none",
        },
      );
      const registrationRecord = registration as {
        active?: { scriptURL?: string } | null;
        scope?: string;
      };
      serviceWorkerDebugState = {
        status: "registered",
        activeScriptURL: registrationRecord.active?.scriptURL ?? null,
        scope: registrationRecord.scope ?? null,
        scriptUrl: INCODEX_SERVICE_WORKER_PATH,
      };
      browserDebugLog("service worker register success", {
        activeScriptURL: registrationRecord.active?.scriptURL ?? null,
        scope: registrationRecord.scope ?? null,
      });
      rememberBrowserDebugEvent("socket", {
      ...
      browserDebugLog("service worker register failed", {
        error: summarizeBrowserError(error),
        scriptUrl: INCODEX_SERVICE_WORKER_PATH,
      });
      rememberBrowserDebugEvent("socket", {
  ...
  function restoreStoredRouteIfNeeded(): void {
    ...
    if (extractLocalConversationIdFromRoute(storedRoute) || isThreadRestoreRoute(storedRoute)) {
      return;
    }
  ...
  function isThreadRestoreRoute(route: string): boolean {
    try {
      const url = new URL(route, window.location.origin);
      return readThreadQueryConversationId(url) !== null;
    } catch {
      return false;
    }
  }
  ...
  function startHeartbeatMonitor(): void {
    ...
    const poll = () => {
      ...
      if (connectionPhase === "connected") {
        setConnectionPhase("degraded", "Incodex connection looks stale. Reconnecting...", {
          mode: "passive",
        });
      }
  ...
  function scheduleReconnect(
    ...
    const nextMessage =
      nextPhase === "reload-required"
        ? "Incodex is still reconnecting. Keep this page open, or refresh it if the connection does not recover."
        : message;
  ...
  function reconnectNow(): void {
    ...
    if (socket && socket.readyState !== WebSocket.CLOSED) {
      ...
      setConnectionPhase("reconnecting", "Reconnecting to Incodex...", {
        mode: "passive",
      });
    ...
    scheduleReconnect("Reconnecting to Incodex...", {
      immediate: true,
      passive: true,
  ...
  function describeReconnectReason(closeEvent?: { code?: number; reason?: string }): string {
    if (!isNetworkOnline()) {
      return "Incodex is offline. Waiting for network...";
    }

    if (!isDocumentVisible()) {
      return "Incodex is paused while this tab is in the background. Reconnecting when it wakes...";
    }

    if (closeEvent?.code === 4000 || closeEvent?.reason === "heartbeat-timeout") {
      return "Incodex connection timed out. Reconnecting...";
    }

    return "Incodex lost the host connection. Retrying...";
  }
  ...
  function flushPendingMessages(): void {
    ...
    while (pendingMessages.length > 0) {
      ...
      const event = {
        queuedRemaining: pendingMessages.length,
      };
      rememberBrowserDebugEvent("queue", event);
      browserDebugLog("websocket flush queued envelope", event);
      socket.send(message);
    }
  }
  ...
  function sendEnvelope(envelope: BrowserToServerEnvelope): void {
    ...
    const summary = summarizeBrowserEnvelope(envelope);
    if (!socket) {
      ...
      rememberBrowserDebugEvent("queue", event);
      browserDebugLog("queue envelope without socket", event);
      void connectSocket();
      return;
    }
    if (socket.readyState === WebSocket.CONNECTING) {
      ...
      rememberBrowserDebugEvent("queue", event);
      browserDebugLog("queue envelope while socket connecting", event);
      return;
    }
    if (socket.readyState !== WebSocket.OPEN) {
      pendingMessages.push(serialized);
      ...
      rememberBrowserDebugEvent("queue", event);
      browserDebugLog("queue envelope while socket unavailable", event);
      void connectSocket();
      return;
    }
    rememberBrowserDebugEvent("out", summary);
    browserDebugLog("send envelope", summary);
  ...
  async function connectSocket(): Promise<void> {
    ...
    setConnectionPhase(
      "reconnecting",
      hasConnected ? "Reconnecting to Incodex..." : "Connecting to Incodex...",
      { mode: "passive" },
    );
    ...
    socket.addEventListener("open", () => {
      ...
      rememberBrowserDebugEvent("socket", event);
      browserDebugLog("websocket open", event);
    ...
    socket.addEventListener("error", () => {
      ...
      rememberBrowserDebugEvent("socket", event);
      browserDebugLog("websocket error", event);
      if (!hasConnected) {
        setConnectionPhase(
          "reload-required",
    ...
    socket.addEventListener("message", (event) => {
      ...
      if (!envelope) {
        browserDebugLog("invalid server envelope", {
          rawType: typeof event.data,
        });
        showNotice("Incodex received invalid server data.");
        return;
      }

      browserDebugLog("receive envelope", summarizeServerEnvelope(envelope));
      rememberBrowserDebugEvent("in", summarizeServerEnvelope(envelope));

      switch (envelope.type) {
        case "bridge_message":
          {
            const bridgeMessage = rewriteBridgeMessageForLocalOverrides(envelope.message);
            if (handleIncodexBridgeMessage(bridgeMessage)) {
              break;
            }
        ...
        case "heartbeat":
          ...
          break;
        case "session_revoked":
          showNotice(envelope.reason || "This Incodex session is no longer available.");
          setConnectionPhase(
            "reload-required",
            envelope.reason || "This Incodex session is no longer available.",
          );
          isClosing = true;
          clearReconnectTimer();
          clearHeartbeatMonitor();
    ...
    socket.addEventListener("close", (event) => {
      ...
      rememberBrowserDebugEvent("socket", closeEventSummary);
      browserDebugLog("websocket close", closeEventSummary);
      if (!shouldReconnect) {
        return;
      }
      const message = isManualReconnect
        ? "Reconnecting to Incodex..."
  ...
  const electronBridge: ElectronBridge = {
    ...
    sendMessageFromView: async (message) => {
      browserDebugLog("electronBridge.sendMessageFromView", summarizeBrowserMessage(message));
      ...
      if (await handleBrowserBackedFetch(message)) {
        return;
      }
    ...
    sendWorkerMessageFromView: async (workerName, message) => {
      browserDebugLog("electronBridge.sendWorkerMessageFromView", {
        workerName,
        message: summarizeBrowserMessage(message),
      });
    ...
    showContextMenu: async () => {
      showNotice("Context menus are not available in Incodex.");
    },
  ...
  window.fetch = (input, init) => {
    ...
    if (url === "vscode://codex/ipc-request") {
      const method =
        ...
        "POST";
      browserDebugLog("rewrite vscode ipc fetch", {
        method,
        url,
      });
  ...
  window.addEventListener("focus", () => {
    publishFocusState();
    handleLifecycleReconnect("Incodex is reconnecting after the page became active.");
  });
  window.addEventListener("blur", publishFocusState);
  window.addEventListener("pageshow", () => {
    handleLifecycleReconnect("Incodex is reconnecting after the page resumed.");
  });
  window.addEventListener("online", () => {
    handleLifecycleReconnect("Incodex is back online. Reconnecting...");
  });
  window.addEventListener("offline", () => {
    setConnectionPhase("degraded", "Incodex is offline. Waiting for network...", {
      mode: "passive",
    });
  });
  document.addEventListener("visibilitychange", () => {
    publishFocusState();
    if (isDocumentVisible()) {
      handleLifecycleReconnect("Incodex is reconnecting after the page became visible.");
    }
  });
