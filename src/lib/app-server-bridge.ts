...
const LOCAL_UNSUPPORTED_FETCH_BODY = {
  error: "unsupported in Incodex",
};
const SLOW_HOST_FETCH_THRESHOLD_MS = 25_000;
const STATSIG_INITIALIZE_HOST = "ab.chatgpt.com";
...
export class AppServerBridge extends EventEmitter implements HostBridge {
  ...
  private readonly queuedFollowUpSendLocks = new Map<string, string>();
    ...
    switch (message.type) {
      ...
      case "fetch-stream":
        this.emit("bridge_message", {
          ...
          error: "Streaming fetch is not supported in Incodex yet.",
        });
        this.emit("bridge_message", {
    ...
    this.emit("worker_message", workerName, {
      ...
      response: {
        ...
        result: {
          type: "error",
          error: {
            message: `Worker "${workerName}" is not available in Incodex yet.`,
          },
        },
      },
    });
      ...
      switch (method) {
        ...
        default:
          return buildIpcErrorResponse(
            requestId,
            `IPC method "${method}" is not supported in Incodex yet.`,
          );
      }
    } catch (error) {
      return buildIpcErrorResponse(
  ...
  private async listWorkspaceDirectoryEntries(params: unknown): Promise<{
    ...
    const workspaceRoot =
      isJsonRecord(params) && typeof params.workspaceRoot === "string"
        ? resolve(params.workspaceRoot)
        : this.getProjectlessWorkspaceRoot();
    const requestedDirectory =
      isJsonRecord(params) && typeof params.directoryPath === "string"
        ? params.directoryPath
        : isJsonRecord(params) && typeof params.path === "string"
          ? params.path
          : workspaceRoot;
    const directoryPath = resolve(requestedDirectory);
    const includeHidden = isJsonRecord(params) && params.includeHidden === true;

    if (!isSameOrChildPath(directoryPath, workspaceRoot)) {
      throw new Error("Directory path must be inside the workspace root.");
    }
    ...
    for (const dirent of dirents) {
      ...
      try {
        ...
        // Ignore broken or inaccessible symlinks in the workspace tree.
      }
    }
    ...
    entries.sort((left, right) => {
      ...
      return compareWorkspaceRootBrowserEntries(left, right);
    });

    const parentPathCandidate = resolve(directoryPath, "..");
    return {
      directoryPath,
      parentPath:
        directoryPath === workspaceRoot || parentPathCandidate === directoryPath
          ? null
          : parentPathCandidate,
      entries,
    };
  ...
  private async handleFetchRequest(message: AppServerFetchRequest): Promise<void> {
    ...
    const slowFetchTimer = setTimeout(() => {
      debugLog("fetch", "host fetch still pending", {
        requestId: message.requestId,
        method,
        url: message.url,
        elapsedMs: Date.now() - startedAt,
      });
    }, SLOW_HOST_FETCH_THRESHOLD_MS);
    slowFetchTimer.unref();

    const logFetchSuccess = (status: number): void => {
      debugLog("fetch", "host fetch success", {
        requestId: message.requestId,
        method,
        url: message.url,
        status,
    ...
    const logFetchError = (status: number, error: string): void => {
      debugLog("fetch", "host fetch error", {
        requestId: message.requestId,
        method,
        url: message.url,
        status,
    ...
    debugLog("fetch", "host fetch start", {
      requestId: message.requestId,
      method,
      url: message.url,
    ...
    try {
      if (message.url === "vscode://codex/ipc-request") {
        ...
        if (controller.signal.aborted) {
          debugLog("fetch", "host fetch aborted after ipc handling", {
            requestId: message.requestId,
            method,
            url: message.url,
            elapsedMs: Date.now() - startedAt,
      ...
      if (message.url.startsWith("vscode://codex/")) {
        ...
        if (controller.signal.aborted) {
          debugLog("fetch", "host fetch aborted after codex host handling", {
            requestId: message.requestId,
            method,
            url: message.url,
            elapsedMs: Date.now() - startedAt,
        ...
        if (handled) {
          if (handled.status < 200 || handled.status >= 300) {
            const errorMessage = readFetchErrorMessage(
              handled.body,
              `Codex host fetch failed for ${new URL(message.url).pathname}.`,
            );
            logFetchError(handled.status, errorMessage);
            this.emitFetchError(message.requestId, handled.status, errorMessage);
            return;
          }
          logFetchSuccess(handled.status);
          this.emitFetchSuccess(message.requestId, handled.body, handled.status);
          return;
        }
        logFetchError(501, `Unsupported Codex host fetch URL: ${message.url}`);
        this.emitFetchError(
          message.requestId,
          501,
      ...
      if (message.url.startsWith("/")) {
        ...
        if (controller.signal.aborted) {
          debugLog("fetch", "host fetch aborted after relative fetch handling", {
            requestId: message.requestId,
            method,
            url: message.url,
            elapsedMs: Date.now() - startedAt,
        ...
        if (controller.signal.aborted) {
          debugLog("fetch", "host fetch aborted after remote relative fetch", {
            requestId: message.requestId,
            method,
            url: message.url,
            elapsedMs: Date.now() - startedAt,
        ...
        logFetchSuccess(handledResponse.status);
        this.emit("bridge_message", {
          type: "fetch-response",
          requestId: message.requestId,
      ...
      const localExternalResponse = handleLocalExternalFetchRequest(message.url);
      if (localExternalResponse) {
        debugLog("fetch", "served local external fetch response", {
          requestId: message.requestId,
          method,
          url: message.url,
          status: localExternalResponse.status,
          elapsedMs: Date.now() - startedAt,
        });
        logFetchSuccess(localExternalResponse.status);
        this.emit("bridge_message", {
          type: "fetch-response",
          requestId: message.requestId,
          responseType: "success",
          status: localExternalResponse.status,
          headers: localExternalResponse.headers,
          bodyJsonString: JSON.stringify(localExternalResponse.body),
        });
        return;
      }
      ...
      const handledResponse = await readRemoteFetchResponse(response);
      if (controller.signal.aborted) {
        debugLog("fetch", "host fetch aborted after remote fetch", {
          requestId: message.requestId,
          method,
          url: message.url,
          elapsedMs: Date.now() - startedAt,
      ...
      if (controller.signal.aborted) {
        debugLog("fetch", "host fetch aborted after error", {
          requestId: message.requestId,
          method,
          url: message.url,
          elapsedMs: Date.now() - startedAt,
      ...
      logFetchError(500, normalized.message);
  ...
  private handleFetchCancel(message: AppServerFetchCancel): void {
    debugLog("fetch", "host fetch canceled", {
      requestId: message.requestId,
    });
  ...
  private async handleCodexFetchRequest(
    ...
    switch (path) {
      ...
      case "queued-follow-up-send-lock-acquire":
        return {
          status: 200,
          body: this.acquireQueuedFollowUpSendLock(body),
        };
      case "queued-follow-up-send-lock-release":
        return {
          status: 200,
          body: this.releaseQueuedFollowUpSendLock(body),
        };
      case "get-settings":
        return {
          status: 200,
      ...
      case "projectless-workspace-root":
        return {
          status: 200,
          body: {
            workspaceRoot: this.getProjectlessWorkspaceRoot(),
          },
        };
      case "projectless-thread-cwd":
        return {
          status: 200,
          body: await this.getProjectlessThreadCwd(body),
        };
      ...
      case "read-file-metadata":
        return await this.readFileMetadata(body);
      case "read-file-binary":
        return await this.readFileBinary(body);
  ...
  private async readFileMetadata(body: unknown): Promise<RelativeFetchResponse> {
    const path = readCodexFilePath(body);
    if (!path) {
      return {
        status: 400,
  ...
  private async readFileBinary(body: unknown): Promise<RelativeFetchResponse> {
    const path = readCodexFilePath(body);
    if (!path) {
      return {
        status: 400,
  ...
  private acquireQueuedFollowUpSendLock(body: unknown): { acquired: boolean } {
    const lock = readQueuedFollowUpSendLockParams(body);
    if (!lock) {
      return { acquired: false };
    }

    const key = buildQueuedFollowUpSendLockKey(lock);
    const existingLockId = this.queuedFollowUpSendLocks.get(key);
    if (existingLockId && existingLockId !== lock.lockId) {
      return { acquired: false };
    }

    this.queuedFollowUpSendLocks.set(key, lock.lockId);
    return { acquired: true };
  }

  private releaseQueuedFollowUpSendLock(body: unknown): Record<string, never> {
    const lock = readQueuedFollowUpSendLockParams(body);
    if (!lock) {
      return {};
    }
    ...
    const key = buildQueuedFollowUpSendLockKey(lock);
    if (this.queuedFollowUpSendLocks.get(key) === lock.lockId) {
  ...
  private getProjectlessWorkspaceRoot(): string {
    if (this.activeWorkspaceRoot && this.workspaceRoots.has(this.activeWorkspaceRoot)) {
      return this.activeWorkspaceRoot;
    }

    return this.cwd.length > 0 ? this.cwd : homedir();
  }

  private async getProjectlessThreadCwd(body: unknown): Promise<{
    cwd: string;
    ...
    const directoryName =
      isJsonRecord(params) && typeof params.directoryName === "string"
        ? params.directoryName
        : isJsonRecord(params) && typeof params.prompt === "string"
          ? params.prompt
          : "";
    const workspaceRoot = join(
      ...
      "Documents",
      "Codex",
      buildProjectlessDirectoryName(directoryName),
    );
  ...
  private async sendLocalRequest(method: string, params?: unknown): Promise<unknown> {
    const id = `incodex-local-${++this.nextRequestId}`;
...
function buildProjectlessDirectoryName(source: string): string {
  const base = source
    .trim()
    .replace(/[\r\n\t]+/g, " ")
    ...
    .trim()
    .slice(0, 48);
  const label = base.length > 0 ? base : "Projectless Chat";
...
function handleLocalExternalFetchRequest(rawUrl: string): RelativeFetchResponse | null {
  let url: URL;
  try {
    url = new URL(rawUrl);
  } catch {
  ...
  if (url.hostname !== STATSIG_INITIALIZE_HOST || url.pathname !== STATSIG_INITIALIZE_PATH) {
    if (url.hostname !== STATSIG_REGISTER_HOST || url.pathname !== STATSIG_REGISTER_PATH) {
      return null;
    }
...
type QueuedFollowUpSendLockParams = {
  conversationId: string;
  lockId: string;
  messageId: string;
};

function readQueuedFollowUpSendLockParams(body: unknown): QueuedFollowUpSendLockParams | null {
  const params = readCodexFetchParams(body);
  if (
    !isJsonRecord(params) ||
    typeof params.conversationId !== "string" ||
    typeof params.messageId !== "string" ||
    typeof params.lockId !== "string"
  ) {
    return null;
  }
...
function buildQueuedFollowUpSendLockKey(lock: QueuedFollowUpSendLockParams): string {
  return `${lock.conversationId}:${lock.messageId}`;
}
