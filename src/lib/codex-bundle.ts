...
export async function resolveDefaultCodexAppPath(): Promise<string> {
  const requestedAppPath = process.env.INCODEX_APP_PATH;
  if (requestedAppPath) {
...
function cacheRootForBuild(
  ...
  return join(homedir(), ".cache", "incodex", directoryName);
}
...
function normalizeCodexAppPath(appPath: string): string {
  ...
  if (isWindowsAbsolutePath(trimmedPath)) {
    if (!isRunningInWsl()) {
      throw new Error(
        "Windows-style Codex app paths are supported when Incodex is running inside WSL.",
      );
    }
